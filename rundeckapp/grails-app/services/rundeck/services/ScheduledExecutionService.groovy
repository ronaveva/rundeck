...
class ScheduledExecutionService implements ApplicationContextAware, InitializingBean, ProjectConfigurable, EventPublisher {
    ...
    static final Logger optionsLogger = Logger.getLogger("com.dtolabs.rundeck.remoteservice.http.options")
     ...
     */
    SessionBinderJobListener sessionBinderListener
...
//                }
    ...
    def deleteScheduledExecution(ScheduledExecution scheduledExecution, boolean deleteExecutions=false,
        ...
        Execution.withTransaction {
            ...
            try {
                jobSchedulesService.persistSchedulesToJob(scheduledExecution.uuid, null, false, scheduledExecution.project)
    ...
    def scheduleJob(ScheduledExecution se, String oldJobName, String oldGroupName, boolean forceLocal=false) {
        ...
        def jobDetail = createJobDetail(se, se.generateJobScheduledName(), se.generateJobGroupName())
    ...
    def Date scheduleCleanerExecutionsJob(String projectName, String cronExpression, Map config) {
        def Date nextTime
        def trigger = jobSchedulesService.createTrigger(projectName, CLEANER_EXECUTIONS_JOB_GROUP_NAME, cronExpression, 1)
...
//                params.notifyPlugin.each { trig, plug ->
    ...
    def _doupdate ( params, UserAndRolesAuthContext authContext, changeinfo = [:] ){
        ...
        if(!failed){
            //TODO: move this to schedules controller
            jobSchedulesService.persistSchedulesToJob(scheduledExecution.uuid, params.schedulesDefinitionDataList, params.scheduleDefinitionsEnabled, params.project)
        }
...
//                scheduledExecution.errors.rejectValue('options', 'scheduledExecution.options.invalid.message')
    ...
    JobDetail createJobDetail(se, String jobname, String jobgroup) {
    ...
    /**
     * Returns true if system executions are active
     * @return boolean
     */
